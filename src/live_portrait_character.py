...
class LivePortraitCharacter:
    ...
    def gen_preset(self, audio):
        ap = AudioProcessor()
        is_speaking = ap(audio)
        is_speaking = torch.nn.functional.pad(is_speaking, (0, 13), mode="constant", value=is_speaking[-1])[13:]
        traj = torch.load("preset/demo_webp/trajectory.pkl")
        while True:
            piui = traj(is_speaking)
            print(len(set(piui)))
            breakpoint()
    ...
    def generate_face_images(self, input_img_path, do_test=False):
        ...
        for i, (p, y, m, e) in enumerate(landmarks):
            ...
            cv2.imwrite(f"preset/demo_webp/frame_{i}.webp", frame, [int(cv2.IMWRITE_WEBP_QUALITY), 20])
            print(f"\r{i} / {len(landmarks)}", end="")
        torch.save(trajectory, f"preset/demo_webp/trajectory.pkl")
        ...
        if do_test:
            ...
            target_frames2 = [cv2.imread("preset/demo_webp/frame_{}.webp".format(f)) for f in traj]
